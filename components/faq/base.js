export default function FAQ({ className = "" }) {
  ...
  return (
    <section
      className={`leading-relaxed max-w-screen-xl mt-12 mx-auto px-4 md:px-8 ${className}`}
    >
      <div className="space-y-3 text-center">
        <h1 className="text-3xl opacity-90 font-semibold">
          Frequently Asked Questions
        </h1>
        <p className="opacity-70 max-w-lg mx-auto text-lg">
      ...
      <div className="mt-14 gap-4 sm:grid sm:grid-cols-2 lg:grid-cols-3">
        {faqsList.map((item, idx) => (
          <div className="space-y-3 mt-5" key={idx}>
            <h4 className="text-xl opacity-80 font-medium">{item.q}</h4>
            <p className="opacity-60">{item.a}</p>
          </div>
        ))}
      </div>
    </section>
