...
export default function Team({ className = "", color = "" }) {
  ...
  return (
    <section id="team" className={`py-14 ${className}`}>
            ...
            {team.map((item, idx) => (
              <li key={idx}>
                ...
                <div className="mt-2">
                  ...
                  <p className={color ? color : "text-primary"}>{item.title}</p>
                  <p className="opacity-70 mt-2">{item.desc}</p>
                  <div className="mt-4 flex justify-center gap-4 opacity-50">
                    <Link href={item.twitter}>
                      <svg
                        className="w-5 h-5 duration-150 hover:opacity-60"
                        fill="currentColor"
                        viewBox="0 0 48 48"
                      >
